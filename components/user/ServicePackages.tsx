...
export const ServicePackages: React.FC<ServicePackagesProps> = ({ setCart,serviceCategory,modelId,fuelType }) => {
  ...
  const [loading, setLoading] = useState(false)
   ...
   const fetchServicePackages = async() => {
    ...
    console.log('the service category',serviceCategory);
const response = await userApi.getServicePackages(modelId,serviceCategory,fuelType);
console.log('the response from fetch service package',response);
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-8 text-center">
          <div className="max-w-md mx-auto">
            ...
            <p className="text-gray-600 mb-6">
              Choose your vehicle to see personalized service packages tailored for your car's needs.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              <div className="bg-white p-4 rounded-lg shadow-sm">
                ...
                <h4 className="font-medium text-gray-800">Customized Services</h4>
                <p className="text-gray-600 mt-1">Services matched to your vehicle model</p>
              </div>
              <div className="bg-white p-4 rounded-lg shadow-sm">
                ...
                <p className="text-gray-600 mt-1">Competitive rates for quality service</p>
              </div>
              <div className="bg-white p-4 rounded-lg shadow-sm">
    ...
    return (
      <div className="lg:col-span-2 space-y-6">
        <div className="flex items-center justify-center py-12">
          ...
          <span className="ml-3 text-gray-600">Loading service packages...</span>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="lg:col-span-2 space-y-6">
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-800 mb-2">No Service Packages Available</h3>
          <p className="text-gray-600">
            We couldn't find any service packages for your selected vehicle. Please try selecting a different vehicle or
            contact support.
          </p>
        </div>
      </div>
        ...
        <div key={service._id} className="bg-white rounded-lg shadow-md overflow-hidden">
          <div
            className={`px-2 py-1 text-white w-max ${
              service._id === "683693d245f412053dec2c98" ? "bg-green-500" : "hidden"
            }`}
          >
            RECOMMENDED
          </div>
            ...
            <div className="flex flex-col md:flex-row">
              ...
              <div className="md:w-3/4 md:pl-6">
                <div className="flex justify-between items-start">
                  <h2 className="text-xl font-semibold">{service.title || "Unnamed Package"}</h2>
                  <div className="bg-gray-100 px-3 py-1 rounded-full text-sm">
                    {service.modelId?.name || "Standard"} {service.fuelType || ""}
                  </div>
                </div>

                <div className="mt-2 text-sm text-gray-600">
                  • {service.brandId?.brandName || "Standard Warranty"} • {service.description || "Recommended service"}
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-4">
                  {(service.servicesIncluded || []).slice(0, 5).map((feature, index) => (
                ...
                {service.servicesIncluded && service.servicesIncluded.length > 5 && (
                ...
                {service.priceBreakup && (
                  <>
                    <span className="text-gray-400 line-through mr-2">
                      Rs. {(service.priceBreakup.total + (service.priceBreakup.discount || 0)).toFixed(2)}
                    </span>
                    <span className="text-2xl font-bold">₹ {service.priceBreakup.total.toFixed(2)}</span>
                  </>
                )}
              </div>
