...
import { X, Plus, Trash2, Upload, ImageIcon, Calculator, Package, Info, AlertTriangle } from "lucide-react"
...
import type { ServicePackageFormData } from "../../validations/servicePackage"
...
const AddServicePackageModal: React.FC<AddServicePackageModalProps> = ({ isOpen, onClose, onSuccess, brands }) => {
  ...
  const onSubmit = async (data: ServicePackageFormData): Promise<void> => {
    setLoading(true)
    try {
      ...
      const imageUrl = await imageUploadApi.uploadImageApi(selectedImage)
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-5xl h-[95vh] p-0 flex flex-col">
        <DialogHeader className="shrink-0 px-6 py-4 border-b bg-gradient-to-r from-blue-50 to-indigo-50">
