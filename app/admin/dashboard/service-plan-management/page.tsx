...
const ServicePlanManagement = () => {
  ...
  const [selectedPackage, setSelectedPackage] =
    useState<IServicePackage | null>(null);
  ...
  useEffect(() => {
    const fetchServicePackages = async () => {
      try {
        setLoading(true);
        const response = await adminApi.getServicePackages(
          searchTerm,
          currentPage,
          statusFilter,
          fuelFilter
        ...
        const err = error as AxiosError<{ message: string }>;
        toast.error(
          err.response?.data?.message || "Failed to fetch service packages."
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleBlockUnblock = useCallback(
    (pkg: IServicePackage, action: ActionType): void => {
      setSelectedPackage(pkg);
  ...
  const confirmBlockUnblock = useCallback(async (): Promise<void> => {
    ...
    try {
      setLoading(true);
      const response = await adminApi.toggleServicePackageStatus(
        selectedPackage._id,
        actionType
      );
      ...
      toast.error(
        err.response?.data?.message ||
          "Failed to update status of service package"
      );
    } finally {
      setLoading(false);
  ...
  const columns: TableColumn<IServicePackage>[] = [
      ...
      render: (_, pkg) => (
        <div className="text-xs lg:text-sm">
          <div className="font-medium">
            {pkg.brandId?.brandName ?? "Unknown Brand"}
          </div>
          <div className="text-gray-600">
            {pkg.modelId?.name ?? "Unknown Model"}
          </div>
        </div>
      ),
    },
    {
      ...
      header: "Fuel Type",
      render: (fuelType) => (
        <TableBadge variant="outline">{fuelType}</TableBadge>
      ),
    },
    {
      key: "services",
        ...
        <div className="text-xs lg:text-sm">
          ...
          {pkg.servicesIncluded.length > 2 && (
            <div className="text-gray-400 text-xs">
              +{pkg.servicesIncluded.length - 2} more
            </div>
          )}
        </div>
      ),
  ...
  const actions: TableAction<IServicePackage>[] = [
    ...
    },
    {
      ...
      onClick: (pkg) =>
        handleBlockUnblock(pkg, pkg.isBlocked ? "unblock" : "block"),
    ...
    <div className="flex-1 md:ml-64 transition-all duration-200 ease-in-out overflow-y-auto">
      ...
      <div className="p-4 md:p-6">
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription>
              {servicePackages.length} packages found
            </CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
      ...
      <ConfirmationDialog
        ...
        message={`Are you sure you want to ${actionType} "${
          ...
          actionType === "block"
            ? "prevent customers from booking"
