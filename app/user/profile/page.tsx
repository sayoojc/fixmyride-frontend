...
import { PlusCircle, Pencil, Trash2, Check, User, MapPin, Car, Shield, Edit } from "lucide-react"
...
const [confirmPassword, setConfirmPassword] = useState("");
      ...
      if (response?.status === 200) {
        ...
        setUser((prevUser) => {
          ...
          return {
            ...prevUser,
            addresses: updatedAddresses,
            defaultAddress: isDeletedDefault ? updatedAddresses[0]?._id || "" : prevUser.defaultAddress,
          }
        })
      } else {
  ...
  const updateProfile = async (phone: string, userId: string, userName: string) => {
    try {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      const response = await userApi.changePasswordApi(user.id, currentPassword, newPassword);
  ...
  const defaultAddressObj = user?.addresses?.find((addr) => addr.isDefault) || user?.addresses?.[0]
  const defaultAddressText = defaultAddressObj ? formatAddress(defaultAddressObj) : ""

  return (
    <motion.div className="lg:col-span-3 container mx-auto py-6" initial="hidden" animate="visible" variants={fadeIn}>
        ...
        <CardHeader className="bg-gradient-to-r from-red-50 to-red-100 pb-6">
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              className={`rounded-full transition-all duration-300 ${isEditing ? "bg-red-600 hover:bg-red-700" : "border-red-300 text-red-700"}`}
            >
              {isEditing ? (
                <>
          ...
          <Tabs defaultValue="profile" className="w-full">
            <TabsList className="w-full justify-start border-b rounded-none bg-transparent h-auto p-0">
              <div className="container flex overflow-x-auto no-scrollbar">
                ...
                {user?.provider === 'local' &&    <TabsTrigger
                  value="security"
                  className="flex items-center py-3 px-4 data-[state=active]:border-b-2 data-[state=active]:border-red-500 data-[state=active]:text-red-500 data-[state=active]:shadow-none rounded-none bg-transparent text-slate-600"
                >
              ...
              >
                <motion.div className="space-y-2" variants={fadeIn}>
                  ...
                  <Input
                    ...
                    className={`bg-slate-50 border-slate-200 focus-visible:ring-red-400 ${isEditing ? "border-red-400" : ""}`}
                    readOnly={!isEditing}
                  />
                </motion.div>
                ...
                <motion.div className="space-y-2" variants={fadeIn}>
                  <Label htmlFor="default-address" className="text-slate-600 text-sm font-medium">
                    Default Address
                  </Label>
                  <Input
                    id="default-address"
            ...
            <TabsContent value="addresses" className="p-6">
              <motion.div className="space-y-4" variants={staggerContainer} initial="hidden" animate="visible">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-xl font-semibold text-slate-800">Saved Addresses</h3>
                  <Button
                ...
                {user?.addresses && user.addresses.length > 0 ? (
                          ...
                          <div className="flex flex-col md:flex-row md:items-center justify-between">
                            <div className="p-4 flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                <span className="font-medium text-slate-800">{address.addressType}</span>
                                {address.isDefault && (
                                  <Badge
                                    variant="outline"
                            ...
                            <div className="flex md:flex-col border-t md:border-t-0 md:border-l border-slate-200 bg-slate-50">
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  address._id && user.id ? handleDeleteAddress(address._id, user.id) : null
                                }}
                              >
                              ...
                              {!address.isDefault && (
                                <>
                                  ...
                                  <Button
                                    ...
                                    onClick={() => {
                                      address._id && user.id ? handleSetDefaultAddress(address._id, user.id) : null
                                    }}
                                  >
                                    <Check className="h-4 w-4 mr-2" /> Set Default
                                  </Button>
                                </>
                              )}
                            </div>
                  ...
                  <motion.div variants={fadeIn} className="text-center py-8">
                    <p className="text-slate-500">No addresses found. Add your first address!</p>
                  </motion.div>
                )}
                <EditAddressModal
            ...
            <TabsContent value="vehicles" className="p-6">
              <motion.div className="space-y-4" variants={staggerContainer} initial="hidden" animate="visible">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-xl font-semibold text-slate-800">My Vehicles</h3>
                  <Button
                  ...
                  <AddVehicleModal open={addVehicleModalOpen} onOpenChange={setAddVehicleModalOpen} setUser = {setUser} user = {user!}/>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {user && user.vehicles.map((vehicle) => (
                        ...
                        <CardContent className="p-0">
                          <div className="flex p-4">
                            ...
                            <div className="flex-1">
                              <h4 className="font-semibold text-slate-800">{vehicle.modelId.name}</h4>
                              <p className="text-sm text-slate-600">{vehicle.brandId.brandName}</p>
                              <p className="text-sm text-slate-600">Reg: {vehicle.registrationNumber}</p>
                              <p className="text-sm text-slate-600">Year: {vehicle.year}</p>
                              <p className="text-sm text-slate-600">fuel: {vehicle.fuel}</p>

                            </div>
                            <div className="flex flex-col space-y-2">
                              <Button variant="ghost" className="h-8 text-red-500 justify-start px-2 hover:bg-red-50">
                                <Pencil className="h-4 w-4 mr-2" /> Edit
                              </Button>
                              <Button
                                variant="ghost"
            ...
            {/* Security Tab */}
            {user?.provider === "local" &&            <TabsContent value="security" className="p-6">
              <motion.div className="space-y-6" variants={staggerContainer} initial="hidden" animate="visible">
                  ...
                  <Card className="border border-slate-200">
                    ...
                    <CardContent className="space-y-4">
                      <div className="space-y-2">
                        <Label htmlFor="current-password" className="text-slate-600 text-sm font-medium">
                          Current Password
                        </Label>
                        <Input
                          id="current-password"
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="new-password" className="text-slate-600 text-sm font-medium">
                          New Password
                        </Label>
                        <Input
                          id="new-password"
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="confirm-password" className="text-slate-600 text-sm font-medium">
                      ...
                      <Button className="bg-red-500 hover:bg-red-600 text-white mt-2"  onClick={handleChangePassword}>Change Password</Button>
                    </CardContent>
                  </Card>
                </motion.div>
                ...
                <motion.div variants={fadeIn}>
                  <Card className="border border-slate-200">
                    <CardHeader>
                      <CardTitle className="text-lg text-slate-800">Two-Factor Authentication</CardTitle>
                    </CardHeader>
                    <CardContent> 
                      <div className="flex items-center justify-between">
