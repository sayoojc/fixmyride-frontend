      ...
      if (axios.isAxiosError(error)) {
        if (error.response?.data) {
          if (typeof error.response.data === "string") {
            // In case the backend sends a string
            errorMessage = error.response.data;
          } else if (typeof error.response.data === "object") {
            // If backend sends JSON with message field
        ...
        // Catch all other errors (non-Axios)
        errorMessage = error.message;
      }
        ...
        <div className="relative bg-gray-900 rounded-2xl p-6 shadow-2xl border border-violet-500/20">
          ...
          <div className="relative z-10">
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-violet-200/70 mt-2 text-sm">Enter the 6-digit code we sent to {email || phone}</p>
            </div>
            ...
            <motion.button
              ...
              className={`w-full py-3 px-4 ${isLoading ? "bg-violet-700/70" : "bg-violet-700 hover:bg-violet-600"} text-white rounded-lg font-semibold transition duration-200 flex items-center justify-center`}
              disabled={isLoading}
            >
              {isLoading ? (
                <>
